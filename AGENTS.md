...
- `designs/` contains the design surface: one dossier per design — `designs/{slug}/design.md` (the target specification) plus, on live dossiers, the required phasing skeleton (`phases/phase-1.md`, `phases/later.md`, `obligations.md`; empty concerns stated, never inferred) and optional `alternatives/` with a `weighing.md` — cataloged in `designs/index.md`, per `schema/design/dossier.md`. Designs are user-instigated: agents draft and propose them in conversation but do not create dossiers unprompted.
...
Designs are not wiki pages. Anything answering "what are we building, and where is it in being built?" — blueprints, operative protocols, process designs, option maps — is a `designs/` dossier governed by lifecycle state (`draft | active | implemented | superseded | abandoned`), not by evidence. `design.md` is a pure target specification; implementation scoping lives in the skeleton, and `phases/` is a complete partition of the design's scope (phase-1 + phase-N + later = everything in `design.md`). Dossier files carry designed types from the surface's own scoped vocabulary (`design | phase | roadmap | alternative | weighing` — `schema/design/dossier.md`), never wiki types. A design fan-out puts candidate variants in the dossier's `alternatives/` with a `weighing.md`; the winner graduates into `design.md`. At a terminal state the whole dossier freezes as the historical record of intent — phase documents retained, nothing dies into anything, and it is not maintained as living documentation of the shipped artifact. Choices with lasting rationale still get wiki `decision` pages.

## Page Format

Every ordinary wiki page uses YAML frontmatter per `schema/wiki/page-format.md`. Required: `title`, `type`, `description`, `sources`, `created`, `timestamp`. `source-capture` pages also require `evidence`; `construct` and `entity` pages require `novelty`; `invariant` pages require `enforcement`. Design-surface files use the dossier frontmatter instead (`schema/design/dossier.md`): `title`, `type` (from the surface's scoped vocabulary), `description`, `created`, `timestamp`, plus `status` on `design.md` — and never `evidence`, `confidence`, or `novelty`.
