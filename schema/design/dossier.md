...
- `design.md` is the only file in a dossier that anything outside the dossier may link to. The standard skeleton files (`phases/`, `later.md`, `obligations.md`, `alternatives/`, `weighing.md`) are durable, typed members of the dossier, but they and any other internal file are workspace from the outside: mutable, internally reorganizable, and never an external link target. Non-standard supporting assets are untyped and outside the skeleton; content that deserves a type belongs in a standard file or an ordinary wiki page.
