...
Agent-agnostic workflow for health-checking the wiki and the design surface. This file is part of the shared `schema/` layer referenced by `AGENTS.md`.
...
- **Invalid type** — `type` field not in the registry: `source-capture`, `construct`, `entity`, `synthesis`, `assessment`, `comparison`, `decision`, `invariant`
...
- **Missing novelty** — `construct` or `entity` pages without a `novelty` field
...
### 🟡 Warnings
- **Confidence above ceiling** — declared `confidence` exceeding the derivation rules in `schema/wiki/page-format.md` (approximated from frontmatter: cited capture tiers, external URL domains, llm-generated grouping)
