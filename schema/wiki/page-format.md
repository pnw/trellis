...
| `type` | Page kind — one of: `source-capture`, `construct`, `entity`, `synthesis`, `assessment`, `comparison`, `decision`, `invariant` (designs are not wiki pages — see `schema/design/dossier.md`) |
...
The wiki separates **source reliability** from **claim credibility**, following the two-axis model used by the NATO/Admiralty grading system, ICD 203, and GRADE (see [[wiki/llm-wiki/syntheses/epistemic-metadata-frameworks]] and [[designs/evidence-tier-schema]]):
...
- Other descriptive page types (construct, entity, synthesis, assessment, comparison) normally cite source-capture pages (e.g., `[[wiki/llm-wiki/sources/karpathy-llm-wiki-knowledge-base-guide]]`).
- Normative pages (decision, invariant) may instead cite the design dossier, capture, or chat that grounds them.
