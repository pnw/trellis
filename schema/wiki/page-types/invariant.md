...
**Instigation:** authored tier — user-instigated or user-ratified (`schema/wiki/page-types/registry.md`, Instigator Tiers). Writing the Removal Path is a ratification act, which is the user's to make; agents draft and propose.
...
**When NOT to use:**
- Not a mere observation that a condition currently holds (that is `entity` or design-dossier content). An invariant requires a cost to breaking it and an analyzed reason not to remove it.
...
- Not an end-to-end description of how a system works (that is a design dossier — `schema/design/dossier.md`). A design's local `Constraints / Invariants` section remains the right home for constraints meaningful only within that one design; promote to an `invariant` page when the constraint is cross-cutting, independently citable, or carries a non-obvious removal analysis.
...
**Type-specific frontmatter:** `enforcement` (required — see `schema/wiki/page-format.md`). Invariant pages carry derived `confidence` like other non-source pages; it rates the support for the Rationale and Removal Path, not the rule itself.
