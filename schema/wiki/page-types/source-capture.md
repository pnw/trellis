...
**Instigation:** capture tier — created by ingest, one per meaningful source; bounded, source-isolated, and delegable to a capture agent with no vault context (`schema/wiki/ingest.md`).
...
**Type-specific frontmatter:** `evidence` (required — see `schema/wiki/page-format.md`). Source-captures do not carry `confidence`.
