...
Design rationale: [[designs/trellis-repo-design]] and [[designs/wiki-federation-and-inheritance]] (in this repo's lab). Adaptation guidance for project wikis specifically: [[designs/project-wiki-application-guide]].
...
2. **Page types.** Which subset of the registry (`schema/wiki/page-types/registry.md`) does this wiki need? Small wikis usually start with `source-capture`, `synthesis`, `decision`; add `construct`/`entity`/`assessment`/`comparison`/`invariant` when the workload warrants. The subset can grow later — adding a type is vendoring one more file.
3. **Design surface.** Does the instance carry a `designs/` surface for its own designs (dossiers per `schema/design/dossier.md`)? Default yes for project wikis (process designs, feature blueprints); a pure reference wiki may omit it.
4. **Link format.** Root-relative wikilinks (default, rename-resilient) or standard markdown relative links (better for plain-GitHub rendering). This choice propagates into the seeded conventions.
5. **Agents.** Which of claude-code / kiro / codex operate this wiki? Determines which adapter files to seed (CLAUDE.md wrapper, `.kiro/` steering, etc.).
6. **Peers.** Which other wikis will this one reference? At minimum usually `trellis` itself (for method cross-references). Record repo URLs.
7. **Optional features.** Local search index (`scripts/qmd-index.sh` + SessionStart hook)? Episodic log (`wiki/episodes.md`)?

## Step 2 — Build the instance

In the target repository:

1. Vendor `schema/` (drop `schema/wiki/page-types/` files for unchosen types; keep `registry.md` intact — it documents the full vocabulary including types not adopted; drop `schema/design/` only if the instance omits the design surface) and `scripts/`.
