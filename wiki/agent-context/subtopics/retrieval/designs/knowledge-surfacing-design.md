...
- The cross-agent Ingest Workflow (`AGENTS.md`, `schema/ingest.md`) now runs this script as a step after every ingest. The cross-agent Query Workflow (`AGENTS.md`, `schema/conventions.md`) now says to try `qmd search`/`qmd query` before falling back to `Grep`/`Glob` — this is the actual "qmd with grep fallback" shape.
