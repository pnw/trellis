...
type: design
description: "Reference layout and rationale for a codebase-specific project wiki instance: coordination meta-files, registry-typed knowledge pages, and topics, reconciled with the schema-inheritance model."
sources:
...
2. **Knowledge pages use registry types.** What earlier drafts called `wiki/decisions/` and `wiki/invariants/` are exactly the registry's `decision` and `invariant` types; a "workflow" page is a `design` page (a process design); an "eval" page is typed by its content — usually `invariant` for standing acceptance rules or `assessment` for validation results.
...
4. A project wiki's manifest records its `page_types` subset; a typical pilot subset is `[source-capture, synthesis, design, decision, invariant]`.
...
| `wiki/designs/` | Type folder (`design`) | System designs and process designs — the old "workflow" pages: `feature-build-loop.md`, `bug-triage-loop.md` |
...
| `wiki/architecture/` | Topic | Stable system maps and boundaries as typed pages: `wiki/architecture/designs/service-boundaries.md` |
...
new project
  ...
  -> add decision/design/invariant/assessment pages as they become necessary
