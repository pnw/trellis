...
title: Roadmap
type: roadmap
description: "Forward-looking backlog for the Trellis method: open design questions, candidate topics, self-maintenance items, and the meta-experiment log."
created: 2026-07-08
...
- First real upgrade: exercise the three-breadcrumb inheritance path — armed 2026-07-13 by the first post-split schema change ([[wiki/llm-wiki/decisions/staged-ingest-and-instigator-tiers]]) — [[wiki/llm-wiki/designs/wiki-federation-and-inheritance]].
...
- Spec frontmatter scope: currently `schema/page-types/` only; decide at the first upgrade exercise whether it extends across the rest of `schema/` — [[wiki/llm-wiki/decisions/staged-ingest-and-instigator-tiers]].
- Typing subsidiary design documents — **resolved 2026-07-14** ([[wiki/llm-wiki/decisions/scoped-subtypes-and-roadmap-type]]): scoped types owned by the parent (`design/phase`, defined in the design spec, outside the registry) plus a top-level `roadmap` type for `later.md`/`obligations.md` and `wiki/roadmap.md`. Path-derived typing was rejected (OKF requires designed types, not path-inferred ones). Remaining open thread: whether a second directory-form type ever justifies partitioning the registry into artifact vs component types — deferred until one appears.
...
- **Design-directory conversion.** The seven live single-file designs convert opportunistically — on next substantive revision or first need for phase/obligation content — via mechanical move to `{slug}/design.md` plus the required skeleton (`phases/phase-1.md`, `phases/later.md`, `obligations.md`) and a `wiki/moves.log` line; no bulk migration ([[wiki/llm-wiki/decisions/design-as-directory]]). Inbound links are unaffected by construction (directory-path link convention).
...
- **Design-lifecycle rule (adopted 2026-07-12, superseded 2026-07-14).** Closed without an evidence verdict: [[wiki/llm-wiki/decisions/designs-finalize-not-die]] repealed the collapse doctrine when the directory form removed its premise. The lifecycle question continues inside the design-as-directory line below.
...
- **Design as directory + finalization lifecycle + scoped types (adopted 2026-07-14, amended same day across two review rounds).** Designs become directories — `design.md` target spec separated from typed subsidiary pages, every standard file required (`phase-1.md`, `later.md`, `obligations.md`; empty concerns stated), `phases/` a complete partition of the design's scope — subsidiary pages carry scoped/planning types (`design/phase`, `roadmap`), and designs finalize as immutable records instead of dying into decisions (`schema/page-types/design.md`, `schema/page-types/roadmap.md`; [[wiki/llm-wiki/decisions/design-as-directory]], [[wiki/llm-wiki/decisions/designs-finalize-not-die]], [[wiki/llm-wiki/decisions/scoped-subtypes-and-roadmap-type]]). Expected: specs stay standalone desired-state documents; phase scoping stops leaking into them; every file has a designed type without registry bloat. Revisit: at the first design created or converted under the rule, and again at the first finalization. Watch for: empty-ceremony required files on protocol-style designs with no implementation surface; `later.md`/`obligations.md` accumulating instead of being pruned as scoped roadmaps; finalized directories getting edited despite the immutability rule; the scoped-type mechanism being reached for casually on other types.
- **Instigator tiers + staged ingest (adopted 2026-07-13).** Who creates which page types and when (`schema/page-types/registry.md`, Instigator Tiers); ingest ends Stage 1 at the capture, Stage 2 reviews vault-aware; lint prints composition stats ([[wiki/llm-wiki/decisions/staged-ingest-and-instigator-tiers]]). Expected: downstream instances stop manufacturing derived pages at ingest; composition becomes observable rather than vibes. Revisit: after the downstream friction reports are ingested and the pilot wiki instantiates under the new guidance — did cold sessions honor occasion-not-justification, and did the middle tier's plural instigators confuse them? Watch for: the tier table hardening into a rail (agents refusing legitimate single-source promotions).
