...
- Scoped types: do artifact kinds diverge per surface (a design-scoped comparison vs a wiki-scoped one), and at what point does the design surface's internal vocabulary deserve legislation? — [[designs/design-surface]] (Obligations).
...
- **Design surface separation (adopted 2026-07-15).** Designs left the wiki for lifecycle-governed dossiers under `designs/` with a split schema ([[wiki/llm-wiki/decisions/separate-design-surface]]; supersedes the 2026-07-12 design-lifecycle rule before its trigger fired — that line is retired here, not verdicted). Expected: design fan-outs (alternatives + weighing) get a native home; the wiki stops stretching evidence machinery over commitments; terminal designs freeze instead of rotting. Revisit: after the next 2–3 design fan-outs — did the dossier shape serve, did the wiki/design boundary and reference-direction rule hold in cold sessions, and did any dossier turn into a knowledge trap?
