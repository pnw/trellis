...
- Typing subsidiary design documents: the owner wants phase/later/follow-ups files to eventually carry their own type, once a structure that makes sense in the design context is defined — but without naive growth of the top-level registry. Candidate mitigations to evaluate against real phase documents from the pilot: design-scoped types outside the registry's decision tree (a second, parent-owned vocabulary such as `design/phase`); path-derived typing (the standard filename *is* the type, no frontmatter enum to maintain); or a single `plan` type covering all subsidiary documents, differentiated by filename — [[wiki/llm-wiki/decisions/design-as-directory]].
...
- **Design as directory + finalization lifecycle (adopted 2026-07-14, amended same day after adversarial review).** Designs become directories — `design.md` target spec separated from untyped subsidiary documents, every standard file required (`phase-1.md`, `later.md`, `follow-ups.md`; empty concerns stated) — and finalize as immutable records instead of dying into decisions (`schema/page-types/design.md`; [[wiki/llm-wiki/decisions/design-as-directory]], [[wiki/llm-wiki/decisions/designs-finalize-not-die]]). Expected: specs stay standalone desired-state documents usable to measure against; phase scoping stops leaking into them; consumers find scope by standard names. Revisit: at the first design created or converted under the rule, and again at the first finalization. Watch for: empty-ceremony required files on protocol-style designs with no implementation surface; `later.md`/`follow-ups.md` accumulating instead of being pruned as scoped roadmaps; finalized directories getting edited despite the immutability rule.
